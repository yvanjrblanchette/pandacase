...
const DesignConfigurator = ({ configId, imageUrl, imageDimensions }: DesignConfiguratorProps) => {
	...
	const [options, setOptions] = useState<{
		...
		matériaux: typeof MATERIALS.options[number];
		finitions: typeof FINISHES.options[number];
	}>({
		color: COLORS[0],
		model: MODELS.options[0],
		matériaux: MATERIALS.options[0],
		finitions: FINISHES.options[0],
	});
	...
	async function saveConfiguration() {
		try {
			...
			toast({
				...
				description: "Un problème est survenu lors de l'enregistrement de votre configuration, veuillez réessayer.",
				variant: "destructive",
			});
		}
	}
			...
			<div className="h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white">
				<ScrollArea className="relative flex-1 overflow-auto">
					...
					<div className="px-8 pb-12 pt-8">
						<h2 className="tracking-tight font-bold text-3xl">Modifier votre coque</h2>
				...
				<div className="w-full px-8 h-16 bg-white">
					...
					<div className="w-full h-full flex justify-end items-center">
						<div className="w-full flex gap-6 items-center">
							<p className="font-medium whitespace-nowrap">{formatPrice((BASE_PRICE + options.finitions.price + options.matériaux.price) / 100)}</p>
							<Button
								isLoading={isPending}
								disabled={isPending}
								...
								onClick={() =>
									saveConfig({
										...
										finish: options.finitions.value,
										material: options.matériaux.value,
