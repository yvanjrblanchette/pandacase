...
import { ArrowRight, Check, FireExtinguisher } from "lucide-react";
...
const DesignPreview = ({ configuration }: { configuration: Configuration }) => {
	...
	const { mutate: createPaymentSession } = useMutation({
		...
		onSuccess: ({ url }) => {
			...
			else throw new Error("Impossible de récupérer l'URL de paiement.");
		},
		onError: () => {
			toast({
		...
		<>
			...
			<div className="mt-20 flex flex-col items-center md:grid text-sm sm:grid-cols-12 sm:grid-rows-1 sm:gap-x-6 md:gap-x-8 lg:gap-x-12">
				...
				<div className="mt-6 sm:col-span-9 md:row-end-1">
					<h3 className="text-3xl font-bold tracking-tight text-gray-900">Votre coque d'{modelLabel}</h3>
				...
				<div className="sm:col-span-12 md:col-span-9 text-base">
					<div className="grid grid-cols-1 gap-y-8 border-b border-gray-200 py-8 sm:grid-cols-2 sm:gap-x-6 sm:py-6 md:py-10">
						<div>
							<p className="font-medium text-zinc-950">Caractéristiques</p>
							<ol className="mt-3 text-zinc-700 list-disc list-inside">
								<li>Compatible avec la recharge sans fil</li>
								<li>Absorption des chocs en TPU</li>
								<li>Emballage fait de matériaux recyclés</li>
								<li>Garantie d'impression de 5 ans</li>
							</ol>
						</div>
						<div>
							<p className="font-medium text-zinc-950">Matériaux</p>
							<ol className="mt-3 text-zinc-700 list-disc list-inside">
								<li>Matériau durable de haute qualité</li>
								<li>Résistant aux rayures et empreintes digitales</li>
							</ol>
						</div>
					</div>
					...
					<div className="mt-8">
						<div className="bg-gray-50 p-6 sm:rounded-lg sm:p-8">
							<div className="flow-root text-sm">
								...
								{finish === "textured" ? (
									<div className="flex items-center justify-between py-1 mt-2">
										<p className="text-gray-600">Fini texturé</p>
								...
								<div className="flex items-center justify-between py-2">
									<p className="font-semibold text-gray-900">Total de la commande</p>
						...
						<div className="mt-8 flex justify-end pb-12">
							<Button onClick={() => handleCheckout()} className="px-4 sm:px-6 lg:px-8">
								Finaliser la commande <ArrowRight className="h-4 w-4 ml-1.5 inline" />
							</Button>
						</div>
					</div>
				</div>
